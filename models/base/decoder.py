...
class Decoder(torch.nn.Module):
    ...
    def fusion(self, x1, x2, x3, x4, fusion_form="concat"):
    ...
    def aggregation_layer(self, fea1, fea2, fea3, fea4, fusion_form="concat", ignore_original_img=True):
        """aggregate features from siamese or non-siamese branches"""

        start_idx = 1 if ignore_original_img else 0
        aggregate_fea = [self.fusion(fea1[idx], fea2[idx], fea3[idx], fea4[idx], fusion_form)
                         for idx in range(start_idx, len(fea1))]

        return aggregate_fea

    # def swin_fusion(self, swinTTrnsomee, x):
    #
    #     x = swinTTrnsomee(x)
    #     return x
